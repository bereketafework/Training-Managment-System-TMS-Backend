...
const { Payments } = require("../db/paymentSchema");
...
router.post("/create", validatePayment, verifyToken, async (req, res) => {
  const data = req.body;
...
router.get("/deleted", verifyToken, async (req, res) => {
  try {
    const result = await db
      .select()
      .from(Payments)
      .where(eq(Payments.Is_deleted, true));
...
router.get("/search/:id", verifyToken, async (req, res) => {
  ...
  try {
    const result = await db
      .select()
      .from(Payments)
      .where(and(eq(Payments.Is_deleted, false), eq(Payments.id, id)));
