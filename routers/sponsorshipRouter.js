...
const validateSponsorShip = require("../validation/sponsorshipValidation");
const { Sponsorships } = require("../db/sponsorshipSchema");
...
router.post("/create", validateSponsorShip, verifyToken, async (req, res) => {
  const data = req.body;
...
router.post("/update/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const existingArray = await db
      .select()
      .from(Sponsorships)
      .where(eq(Sponsorships.id, UsersId));
...
router.get("/deleted", verifyToken, async (req, res) => {
  try {
    const result = await db
      .select()
      .from(Sponsorships)
      .where(eq(Sponsorships.Is_deleted, true));
...
router.get("/all", verifyToken, async (req, res) => {
  try {
    const result = await db
      .select()
      .from(Sponsorships)
      .where(eq(Sponsorships.Is_deleted, false));
...
router.get("/search/:id", verifyToken, async (req, res) => {
  ...
  try {
    const result = await db
      .select()
      .from(Sponsorships)
      .where(and(eq(Sponsorships.Is_deleted, false), eq(Sponsorships.id, id)));
    if (result.length === 0) {
      return res.status(404).json({ message: "No data available" });
    }
    res.status(201).json(result);
