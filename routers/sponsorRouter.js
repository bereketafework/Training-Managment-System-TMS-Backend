...
const validateSponsor = require("../validation/sponsorValidation");
const { Sponsors } = require("../db/sponsorSchema");
...
router.post("/create", validateSponsor, verifyToken, async (req, res) => {
  const data = req.body;
...
router.get("/deleted", verifyToken, async (req, res) => {
  try {
    const result = await db
      ...
      .from(Sponsors)
      .where(eq(Sponsors.Is_deleted, true))
...
router.get("/all", verifyToken, async (req, res) => {
  try {
    const result = await db
      ...
      .from(Sponsors)
      .where(eq(Sponsors.Is_deleted, false))
      .orderBy(Sponsors.Name);
    if (result.length === 0) {
      return res.status(404).json({ message: "No data available" });
    }
...
router.get("/search/:id", verifyToken, async (req, res) => {
  ...
  try {
    const result = await db
      .select()
      .from(Sponsors)
      .where(and(eq(Sponsors.Is_deleted, false), eq(Sponsors.id, id)));
    if (result.length === 0) {
      return res.status(404).json({ message: "No data available" });
    }
    res.status(201).json(result);
