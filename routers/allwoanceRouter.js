...
const { Allwoances } = require("../db/allwoanceSchema");
const validateAllwoance = require("../validation/allwoancesValidation");
...
router.post("/create", validateAllwoance, verifyToken, async (req, res) => {
  const data = req.body;
...
router.post("/update/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const existingArray = await db
      .select()
      .from(Allwoances)
      .where(eq(Allwoances.id, UsersId));
...
router.get("/deleted", verifyToken, async (req, res) => {
  try {
    const result = await db
      .select()
      .from(Allwoances)
      .where(eq(Allwoances.Is_deleted, true));
    // if (result.length === 0) {
...
router.get("/all", verifyToken, async (req, res) => {
  try {
    const result = await db
      .select()
      .from(Allwoances)
      .where(eq(Allwoances.Is_deleted, false));
...
router.get("/search/:id", verifyToken, async (req, res) => {
  ...
  try {
    const result = await db
      .select()
      .from(Allwoances)
      .where(and(eq(Allwoances.Is_deleted, false), eq(Allwoances.id, id)));
